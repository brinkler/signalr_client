...
class ServerSentEventsTransport implements ITransport {
  ...
  ServerSentEventsTransport(SignalRHttpClient httpClient, AccessTokenFactory? accessTokenFactory, Logger? logger, bool logMessageContent)
  ...
  Future<void> connect(String? url, TransferFormat transferFormat) async {
    ...
    if (transferFormat != TransferFormat.Text) {
      return Future.error(GeneralError("The Server-Sent Events transport only supports the 'Text' transfer format"));
    }
    ...
    _sseClient!.stream.listen((data) {
      if (onReceive != null) {
        try {
          _logger?.finest('(SSE transport) data received. ${getDataDetail(data, _logMessageContent)}.');
          onReceive!(data);
        } catch (error) {
          _close(error: error);
          return;
